...
function generateName(target, cfg, cfgPh, obj, objPh) {
  var out = [],
      ...
      amountUp = Math.abs(cnt.up),
      amountDown = Math.abs(cnt.down);

  for (var i = 0, len = obj.state.length; i < len; i++) {
    var newName = findAndReplace(cfgPh, obj, i),
        isPre = obj.state[i][0],
        isSuff = obj.state[i][2],
        cntUp = Math.abs(cnt.up, amountUp.toString().length),
        cntDown = Math.abs(cnt.down, amountDown.toString().length); 
